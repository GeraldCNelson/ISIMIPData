...
climateVars <- c("tave", "tasmax", "tasmin", "tave", "pr", "hurs") 
#climateVars <- c( "tave") 
...
#test value
k <- "ssp585"
m = 1 # regionlookup row 1, location in Norway
...
for (m in 1:nrow(regionInfoLookup)) {
  locLat <- regionInfoLookup[m, latitude]
  locLong <- regionInfoLookup[m, longitude]
  locExtent <- ext(locLong - extentRange, locLong + extentRange, locLat - extentRange, locLat + extentRange)
  locText_region <- regionInfoLookup[m, region]
  locText_country <- regionInfoLookup[m, country]
  locText <- paste0(locText_region, ",\n ", locText_country)
  
  for (k in sspChoices) {
    for (l in startyearChoices_ensemble) {
      yearSpan <- paste0(l, "_", l + yearRange)
      print(paste0("ssp choice: ", k, ", start year: ", l, ", region: ", locText))
      for (j in climateVars) {
        varName <- j
        print(paste0("varname: ", varName))
        fileNameMean <- paste0("data/cmip6/annualMean/ensembleAnnualMean_", j,  "_",  yearSpan, "_", k, ".tif") 
        fileNameCV <- paste0("data/cmip6/annualMean/ensembleAnnualCV_", j,  "_",  yearSpan, "_", k, ".tif")
        #      fileNameSD <- paste0("data/cmip6/annualMean/regions/ensembleAnnualSD_", j,  "_",  yearSpan, "_", k, ".tif")
        ...
        meanData_region <- crop(meanData, locExtent)
        CVData_region <- crop(CVData, locExtent)
        # plot Ensemble mean
        ...
        meanData_region <- raster::brick(meanData_region)
        
        g <- levelplot(meanData_region, main = titleText, col.regions = col.l, at = myat, par.settings = mapTheme, 
        ...
        g <- g + panel.pointLabel(x = locLong, y = locLat, labels = locText)

        
        g <- g + text(locLong, locLat, locText, pos = 1, cex = 0.7) # pos = 1 is below
        
        plotFileName <- paste0("graphics/cmip6/annualMean/regions/regions/ensembleAnnualMean_",  varName, "_",  yearSpan, "_", k, ".jpg")
        ...
        g$legend$top <- g$legend$right <-  NULL # get rid of the graphs above and to the right of a single layer levelplot
        g <- g + latticeExtra::layer(sp.polygons(coastsCoarse.Rob, col = "black", lwd = 0.5))
        plotFileName <- paste0("graphics/cmip6/annualMean/regions/ensembleAnnualCV_",  varName, "_",  yearSpan, "_", k, ".jpg")
        print(paste0("plot file name: ", plotFileName, " for climate variable ", varNameLong))
        jpeg(plotFileName, width = jpgWidth, height = jpgHeight, quality = 100, units = "in", res = 300)
        print(g)
        dev.off()
...
for (j in climateVars) {
  ...
  varNameLong <- as.character(varNamesInfo[variableShortName %in% varName, variableLongName])
  varNameLongUnits <- as.character(varNamesInfo[variableShortName %in% varName, units])
  fileNameMean <- paste0("data/cmip6/annualMean/regions/annualMean_", j, "_observed_", yearSpan, ".tif") 
  ...
  plotFileName <- paste0("graphics/cmip6/annualMean/regions/annualMean_",  varName, "_observed_",  yearSpan, ".jpg")
  jpeg(plotFileName, width = jpgWidth, height = jpgHeight, quality = 100, units = "in", res = 300)
  print(g)
  dev.off()
