...
#' }
#' 
calZenithR <- function(dates,lon,lat, hour=FALSE){
  ...
  if(hour){
    ...
    utc_hour <- as.numeric(format(d1,'%H'))
  } else {
 ...
 # if (is_leapyear(year)) dpy=366 else dpy=365 converted to vectorized below
  ...
  TrueSolarTime <- (utc_hour*60)+TimeOffset
  ...
  # if (CosZen > 1.0)  CosZen <- 1.0 convert if to vector below
  # if (CosZen < -1.0) CosZen <- -1.0
  CosZen[CosZen > 1.0] <- 1.0
  CosZen[CosZen < -1.0] <- -1.0
