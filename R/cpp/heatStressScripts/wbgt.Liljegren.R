...
wbgt.LiljegrenR <- function(tas, dewp, relh, wind, radiation, dates, lon, lat, tolerance=1e-4,noNAs=TRUE, swap=FALSE, hour=FALSE) {
 ...
 # relh <- dewp2hurs(tas,dewp) # input in degC, output in % original
  ...
  for (i in which(xmask)){
    ...
    zenithDeg <- calZenithR(dates[i], lon, lat, hour)
    ZenithAngle <- degToRad(zenithDeg)
    
    # Calculate globe temperature
    Tg[i] <- fTgR(tas[i], relh[i], Pair, wind[i], MinWindSpeed, radiation[i], 
                 propDirect, ZenithAngle, tolerance=tolerance)
    
    # Calculate natural wet bulb temperature
    Tnwb[i] <- fTnwbR(tas[i], dewp[i], relh[i], Pair, wind[i], MinWindSpeed, 
