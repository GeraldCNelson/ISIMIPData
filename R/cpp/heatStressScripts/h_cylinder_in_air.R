...
h_cylinder_in_airR <- function(Tk, Pair, speed, min.speed, diam.wick){
  ...
  therm.con <- thermal_condR(Tk)
  
  # Density of the air
  density <- Pair * 100 / (r.air * Tk)
  
#  if(speed < min.speed) speed <- min.speed original
  speed[speed < min.speed] <- min.speed 
   
  # Reynolds number
  Re <- speed * density * diam.wick / viscosityR(Tk)
  # print("Re ---")
  # print(Re)
