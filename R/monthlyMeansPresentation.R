...
titleString <- paste0("Monthly Means of Minimum and Maximum Temperature, Relative Humidity and Precipitation ")
contentString <- paste0("Preliminary Results: Monthly ensemble means and coefficients of key climate variables for four time periods to 2100. Powerpoint produced on ", Sys.Date())
...
IntroText0 <- "The climate variables daily minimum and maximum temperature (tmin or tasmin and tmax or tasmax), relative humidity (rh or hurs), and precipitation (pr) are the key climate variables for agricultural activities.  "
...
IntroText4 <- "The values are monthly means for observed data for 2001-2010 and  ensemble means and coefficients of variation for three 10 year periods (2021-2030, 2051-2060, and 2091-2100)."
  ...
  for (j in climateVars) {
   ...
   # add_slide(my_pres, layout = 'Title Only', master = 'Office Theme') %>% 
    ...
    for (l in startyearChoices_ensemble) {
      ...
      fileNameMean <- paste0("graphics/cmip6/monthlyMean/ensembleMonthlyMean_",  varName, "_",  yearSpan, "_", k, ".jpg")
...
print(my_pres, target = "presentations/cmip6/monthlyMean/monthlyMeansCVs.pptx") %>% browseURL()
