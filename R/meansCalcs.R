    ...
    for (l in startyearChoices) {
      for (j in climateVars) {
        ...
        if (j %in% "pr") {
          ...
          print(system.time(rast.monthlyMean <- tapp(rastIn, indices_mean_month, fun = sum))) # monthly sum of precip. Called monthlyMean to fit with the rest of the code
...
for (j in observedlist) {
  ...
  if (j %in% "pr") {
    ...
    indices_mean_month <- as.numeric(indices_mean_month)
    print(system.time(rast.monthlyMean <- tapp(pr, indices_mean_month, fun = sum)))
  } else {
...
# do same calculations on historical data; note ensemble calculations for historical done else. Here we just need to calculate the monthly means
climateVars <- c("hurs", "tasmax", "tasmin", "pr", "tave", "rsds", "sfcwind")
#observedlist <- c( "pr")
startyearChoices <-  c(1971, 1981, 1991) #2011, 2041, 2051, 2081) # c(2091) # c(2006) #, 2041, 2051, 2081)
...
#  indices <- paste0("X", as.character(indices))
  ...
  fileNameOut_annualMean <- paste0("annualMean_", j, "_ensemble_historical_", yearSpan, ".tif")
  fileNameOut_monthlyMean <- paste0("monthlyMean_", j, "_ensemble_historical_", yearSpan, ".tif")
