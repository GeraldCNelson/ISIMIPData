...
for (j in variableChoices) {
  ...
  fileNameCV <- paste0("data/cmip6/monthlyMean/ensembleMonthCV_", j,  "_",  yearSpan, "_", k, ".tif")
...
#      fileNameSD <- paste0("data/cmip6/monthlyMean/ensembleMonthSD_", j,  "_",  yearSpan, "_", k, ".tif")
   ...
   #   SDData <- rast(fileNameSD)
     ...
     if (j %in% c("tasmax", "tasmin")) myat <- c(0, 1, 2, 3, 4,  maxVal_CV_j)
      if (j %in% "hurs") myat <- c(0, 1, 2, 5, 10,  maxVal_CV_j)
      if (j %in% "pr") myat <- c(0, 1, 5, 10, 20, maxVal_CV_j)
      
      my.brks=seq.int(0, maxVal_CV_j, length = 6)
      myColorkey <- list(at=my.brks, labels=list(at=my.brks, labels=myat), space="right")
