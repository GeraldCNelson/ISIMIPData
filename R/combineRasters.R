...
climateVars <- c("_tasmin_", "_tasmax_", "_tas_", "_pr_", "_hurs_", "_rsds_", "_sfcwind_", "_ps") 
climateVarstoKeep <- c("_pr_") # "tasmax", "tasmin"
...
# get list of files already done  
fileList_dest <- list.files(paste0(locOfFiles_out), full.names = TRUE, recursive = TRUE)
fileList_dest <- fileList_dest[!grepl("tif.aux.xml", fileList_dest, fixed = TRUE)]
...
for (k in sspChoices) {
  for (i in modelChoices) {
    ...
    for (j in climateVarstoKeep) {
        fileName_out <- paste0(locOfFiles_out,  tolower(i), j,  k, "_", yearSpanNew, ".tif")
        #      unlink(fileName_out) # use this when you want fileName_out to be redone
        if (!fileName_out %in% fileList_dest) {
          gc()
          ...
          fileList <- fileList[!grepl("tif.aux.xml", fileList, fixed = TRUE)]
           filestoKeepclim <- fileList[grep(j, fileList, fixed = TRUE)]
           filestoKeepClimModel <- filestoKeepclim[grep(i, filestoKeepclim, fixed = TRUE)]
           filestoKeepClimModelssp <- filestoKeepClimModel[grep(k, filestoKeepClimModel, fixed = TRUE)]
           filestoKeepY1 <- filestoKeepClimModelssp[grep(yearSpanOld1, filestoKeepClimModelssp, fixed = TRUE)] # keep only file names with the correct start year
          filestoKeepY2 <- filestoKeepClimModelssp[grepl(yearSpanOld2, filestoKeepClimModelssp, fixed = TRUE)] # keep only file names with the correct start year
          if (!length(filestoKeepY1) == 1) stop("something wrong with filestoKeepY1")
          if (!length(filestoKeepY2) == 1) stop("something wrong with filestoKeepY2")
          r1 <- rast(filestoKeepY1)
          r2 <- rast(filestoKeepY2)
          rout <- c(r1, r2)
          if (j == "_pr_") print(system.time(rout <- rout * 86400))
          if (j == c("_tasmin_", "_tasmax_", "_tas_")) print(system.time(rout <- rout - 273.15))
          print(rout)
          startDate <- paste0(l, "-01-01"); endDate <- paste0(l + yearRangeNew, "-12-31")
          indices <- seq(as.Date(startDate), as.Date(endDate), 1)
          indices <- paste0("X", as.character(indices))
          names(rout) <- indices
          print(system.time(writeRaster(rout, filename = fileName_out, overwrite = TRUE,  wopt= woptList))); flush.console()
          rout <- NULL
          gc()
        }
      }
  ...
  # rout <- rast(paste0(locOfFiles, "/ssp585/UKESM1-0-LL/ukesm1-0-ll_r1i1p1f2_w5e5_ssp585_tasmin_global_daily_2041_2050.nc"))
