...
f_ensembleCalcs <- function(k, l, j) {
  ...
  writeRaster(ras.test.mean, filename = fileName_out_mean, format = "GTiff", overwrite = TRUE, wopt=list(gdal="COMPRESS=LZW"))
  writeRaster(ras.test.cv, filename = fileName_out_cv, format = "GTiff", overwrite = TRUE, wopt=list(gdal="COMPRESS=LZW"))
