  ...
  for (j in thiListReduced) {
    ...
    for (l in startYearChoices) {
      ...
      if (l %in% startYearChoices[1]) { 
        fileNameMean <- paste0(lofOfGraphicsFiles, "regionInfo/THI/", j, "MonthlyAve_",  yearSpan, "_", region, ".png")
      ...
      if (!l %in% startYearChoices[1]) {
        for (k in sspChoices) {
          ...
          fileNameMean <- paste0(lofOfGraphicsFiles, "regionInfo/THI/", j, "MonthlyAve_", k, "_",  yearSpan, "_", region, ".png")
          extImgMean <- external_img(src = fileNameMean, width = 5, height = 5)
