...
for (i in 1:nrow(regionInfoLookup)) {
  ...
  my_pres <- read_pptx(path = "data-raw/basepptx.pptx") %>% 
  ...
  for (j in climateVars) {
    ...
    ensembleTitle <- paste0("Ensemble monthly mean for ", varNameLong)
    ensembleTextPost <- paste0(" The next 7 slides show its value for the base period 2001-2010"," and for the scenario periods ", "2021-2030, 2051-2060, and 2091-2100. For the scenario periods, results are shown for SSP126 and SSP585. The legends are common across figures and indicate the range of values across all the time periods. The values in each cell are available upon request in the .csv format. ")
    if (varName %in% "hurs")  ensembleText <- paste0("The climate variable called ", j, " is ", varNameLong, ". It is a measure of how much water is in the air relative to the maximum amount possible. In a cold climate region, the maximum is very small so relative humidity is usually very high. In a warm climate region the maximum is high so relative humidity can vary subtantially between 0 and 100.", ensembleTextPost)
