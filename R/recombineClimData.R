...
for (k in varsToCombine) {
  for (l in startYears) {
    print(paste0("working on: directory ", dirList[i], ", variable ", k, ", start year ", l))
    yearSpan_r1 <- paste0(l, "_", l + yearRangeOld)
    yearSpan_r2 <- paste0(l+10, "_", l+10 + yearRangeOld)
    yearSpan_r3 <- paste0(l+20, "_", l+20 + yearRangeOld)
    ...
    print(system.time(writeRaster(rNew, outFileName,  overwrite = TRUE, wopt= woptList))); flush.console()
  }
}
...
for (j in varsToCombine) {
  ...
  file.copy(from = fileIn, to = fileout)
}
