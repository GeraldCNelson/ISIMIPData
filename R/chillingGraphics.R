...
l <- 2091
m = "apple"
pdf.options(with = 8, height = 8, reset = FALSE)
...
for (k in sspChoices) {
  for (l in startyearChoices_ensemble) {
    for (m in fruits) {
      ...
      plotFileName <- paste0(getwd(), "/graphics/cmip6/chillingHours/", "chillHrs_NorthernHem_ensembleMean_masked_", cropName, "_", yearSpan, "_", k, ".pdf")
      pdf(plotFileName)
      ...
      #      myat[2] <- minChillHours; myat[3] <- maxChillHours
      plotFileName <- paste0(getwd(), "/graphics/cmip6/chillingHours/", "chillHrs_NorthernHem_ensembleCV_masked_", cropName, "_", yearSpan, "_", k, ".pdf")
      pdf(plotFileName)
      ...
      myat <- round(seq.int(from = 0, to =  maxMean, length.out = 5))
      myat[2] <- minChillHours; myat[3] <- maxChillHours
      plotFileName <- paste0(getwd(), "/graphics/cmip6/chillingHours/", "chillHrs_SouthernHem_ensembleMean_masked_", cropName, "_", yearSpan, "_", k, ".pdf")
      pdf(plotFileName)
      ...
      plotFileName <- paste0(getwd(), "/graphics/cmip6/chillingHours/", "chillHrs_SouthernHem_ensembleCV_masked_", cropName, "_", yearSpan, "_", k, ".pdf")
      pdf(plotFileName)
...
for (m in fruits) {
  ...
  myat <- round(seq.int(from = 0, to =  maxMean, length.out = 5))
  myat[2] <- minChillHours; myat[3] <- maxChillHours
  plotFileName <- paste0(getwd(), "/graphics/cmip6/chillingHours/", "chillHrs_NorthernHem_masked_", cropName, "_observed_",  yearSpan, ".pdf")
  ...
  plotFileName <- paste0(getwd(), "/graphics/cmip6/chillingHours/", "chillHrs_SouthernHem_masked_", cropName, "_observed_",  yearSpan, ".pdf")
  pdf(plotFileName)
  g <- levelplot(chillHrsSouthernHem_mean, col.regions = col.regionsDefault, margin = FALSE,
                 at = myat, par.settings = mapTheme, 
                 colorkey = list(at = myat),
  ...
  for (k in sspChoices) {
    for (l in startyearChoices) {
      for (m in 1:nrow(IPCC_WG2_Ch5_crop_temperature_table)) {
        ...
        plotFileName <- paste0("graphics/cmip6/optTempRange/optTempRange_", cropName, "_", lowerOpt, "_", upperOpt, "_",  modelName.lower, "_", k,  "_", yearSpan, ".pdf")
        if (l == 2001) plotFileName <- paste0("graphics/cmip6/optTempRange/optTempRange_", cropName, "_", lowerOpt, "_", upperOpt, "_observed_", yearSpan, ".pdf")
        pdf(plotFileName)
