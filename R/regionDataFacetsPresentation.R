...
variableChoices <- c("tave", "tasmax", "tasmin", "pr", "hurs") 

variableChoices <- "tave"
...
for (i in 1:(nrow(regionInfoLookup) - 1)) {
  ...
  meanDataFile <- paste0("data/regionResults/meanTemp_", region, "_", country, ".csv")
  regionMeanHolder <- as.data.table( read.csv(meanDataFile))
  regionMeanHolder[, X := NULL]
  colsToRound <- c("meanTemp_2021", "meanTemp_2051", "meanTemp_2091")
  regionMeanHolder <- regionMeanHolder[,(colsToRound) := round(.SD,1),.SDcols = colsToRound]
  ...
  titleString <- paste0("Ensemble mean annual temperature (°C) for ", region, ", ", country)
  contentString <- paste0("Ensemble means for three time periods to 2100. Powerpoint produced ", Sys.Date())
  ...
  my_pres <- add_slide(my_pres, layout = "Title and Content", master = "Office Theme") %>%
    ph_with(value = "Ensemble-based annual mean values by location", location = ph_location_type(type = "title")) %>%
  ...
  for (k in sspChoices) {
    ...
    for (j in variableChoices) {
      ...
      ensembleTitle <- paste("Ensemble mean for average annual temperature")
      ...
      for (l in startyearChoices_ensemble) {
        ...
        add_slide(my_pres, layout = 'Title Only', master = 'Office Theme') %>% 
          ph_with(value = extImgMean, location = ph_location(left = .5, top = .5, width = 7, height = 7) ) # %>%
        #      ph_with(value = extImgCV, location = ph_location(left = 5, top = 0, width = 5, height = 8) )
      }
    }
  }
  outFilename <- paste0("presentations/cmip6/regionInfo/annualMeans_temp", "_", region, ", ", country, ".pptx")
