# calculate animal and human climate stress
{
  ...
  f_THI <- function(k, l, yearRange) {
    ...
    for (modelChoice in modelChoices) {
      modelName.lower <- tolower(modelChoice)
      ...
      for (breedNum in 1:length(breedChoice)) {
        ...
        if (speciesName %in% c("generic", "bos", "chicken", "sheep", "pigs", "cattle")) { # goats removed from this list because it uses the same THI formula
          cols <- colorList
        ...
        extremeStress <- bpList[breeds %in% breedName, extremeStress]
...
f_ensemble_THI <- function(k, l, yearSpan) {
  for (m in breedChoice) {
    print(paste0("------", "------"))
    ...
    cat(paste0(red("breed: ", m, ", ensemble ssp: ", k, ", start year: ", l, "\n"))) #, ", ratio of small to large: ", round(ratio, 2)), "\n"))
