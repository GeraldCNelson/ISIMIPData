  ...
  for (modelChoice in modelChoices)  {
    for (l in startyearChoices) {
      yearSpan <- paste0(l, "_", l + yearRange)
       ...
       for (cropChoice in get(cropChoices[o])) {
          print(paste0("crop: ", cropChoice))  
          modelName.lower <- tolower(i)
          fileName_ggd_in <- paste0(locOfgddsFiles, modelName.lower, "_", cropChoice, "_", k, "_gdd", "_global_daily_", yearSpan, ".tif")
          gdd <- rast(gddIn_crop)
          names(gdd) <- layerNames
          indices <- format(as.Date(names(gdd), format = "X%Y.%m.%d"), format = "%j") # %j is day of the year
          indices <- as.numeric(indices)
          
          print(system.time(gdd_north <- crop(gdd, extent_NH)))
                print(system.time(gdd_south <- crop(gdd, extent_SH)))
          ...
          cropCalendarName <- ann_crop_temp_table[crop %in% cropChoice, crop.calendar]
          cropCalFilesLoc <- paste0("data-raw/crops/cropCalendars/ALL_CROPS_netCDF_0.5deg_filled/")
          fileName_in <- paste0(cropCalFilesLoc, cropCalendarName, ".crop.calendar.fill.nc")
          ...
          croppingCalendar_plant_crop_north <- crop(croppingCalendar_plant, extent_NH)
          croppingCalendar_harvest_crop_north <- crop(croppingCalendar_harvest, extent_SH)
